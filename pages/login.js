  ...
  async function handleSubmit() {
    ...
    if (mode === "login") {
      ...
      const { error } = await supabase.auth.signUp({
        email, password,
        options: { emailRedirectTo: "https://mystorija.com/app" }
      });
      if (error) { setError(error.message); setLoading(false); return; }
      ...
      const { error } = await supabase.auth.resetPasswordForEmail(email, {
        redirectTo: "https://mystorija.com/login?mode=newpassword"
      });
      if (error) { setError(error.message); setLoading(false); return; }
    ...
    <script>
      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <p style={{ fontSize: 14, color: C.muted, marginTop: 6 }}>KI-Renovierung für dein Zuhause</p>
        </div>
        ...
        <div style={{ background: C.card, borderRadius: 20, padding: 32, border: `1px solid ${C.border}`, boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }}>
          ...
          <p style={{ fontSize: 13, color: C.muted, marginBottom: 24 }}>
            {mode === "login" ? "Melde dich an um weiterzumachen" : mode === "signup" ? "Erstelle dein Mystorija-Konto" : "Wir senden dir einen Reset-Link"}
          </p>
          ...
          {mode !== "reset" && (
            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: C.muted, display: "block", marginBottom: 6 }}>PASSWORT</label>
              <input
          ...
          >
            {loading ? "Laden..." : mode === "login" ? "Anmelden →" : mode === "signup" ? "Konto erstellen →" : "Reset-Link senden →"}
          </button>
        ...
        <div style={{ textAlign: "center", marginTop: 20 }}>
          <a href="/app" style={{ fontSize: 13, color: C.muted, textDecoration: "none" }}>
            Ohne Konto fortfahren (eingeschränkt) →
          </a>
        </div>
      </div>
    </>
